...
export class ProjectStore implements TaskSource {
    ...
    try {
      ...
      if (file instanceof TFile) {
        ...
        await this.app.vault.process(file, (content) => {
          ...
          // The on-disk fm description is what the last save wove into the body;
          // stripping with it (not the possibly-edited in-memory one) keeps an
  ...
  async duplicateTask(project: Project, sourceId: string, includeSubtasks: boolean): Promise<Task | null> {
    ...
    // (a duplicate is new work — never a closed-without-verdict incident).
    const copy = cloneTaskSubtree(source, includeSubtasks, this.statusesFor(project))
  ...
  /**
   * A move whose destination lies inside the moved task's own subtree would
   * nest the tree in itself — every recursive walk (render, save, flatten)
   * then hangs Obsidian. Defense-in-depth behind the drop-target guards.
   */
  private wouldCreateCycle(task: Task, newParentId: string | null): boolean {
    if (!newParentId) return false
    return newParentId === task.id || findTask(task.subtasks, newParentId) !== null
  }
  ...
  async moveTasks(project: Project, taskIds: string[], newParentId: string | null): Promise<void> {
    for (const id of taskIds) {
      const task = findTaskById(project, id)
      if (!task) continue
      if (this.wouldCreateCycle(task, newParentId)) continue
  ...
  private stampActivity(project: Project, task: Task, patch: Partial<Task>): void {
    ...
    // Activity is store-owned (appendActivity/stampActivity only): the patch's
    // copy is a stale clone — an entry appended to the live task while an
    // editor was open (an SLA breach) must survive the editor's whole-clone
    // save, so the merge bases on the LIVE log and patch.activity is dropped.
    if (entries.length) {
      patch.activity = [...task.activity, ...entries]
    } else {
  ...
  async updateTask(
    ...
    // or reorder subtasks — but its clone can be STALE: a subtask created via
    // the board while the editor was open is absent from it. Destructive
   ...
   * a write, and trash the files of explicitly removed subtasks. By the time
   * this runs, mergeMissingSubtasks has already re-attached every live subtask
   * the patch merely didn't know about, so an id missing here was removed with
   * intent (updateTask's removedSubtaskIds). Unchanged subtasks aren't rewritten.
   */
