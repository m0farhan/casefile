...
import { openHeaderAnalysis } from './modals/HeaderAnalysisModal'
import { openToolbox } from './modals/ToolboxModal'

/** The generated board backlink, as a whole line. Same shape stripAutoGeneratedContent removes on read. */
const BOARD_BACKLINK_RE = /^Project: \[\[.*\]\]$\n?/gm
...
export default class PMPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.addCommand({
      id: 'strip-board-backlinks',
      name: 'Remove board backlinks from task notes',
      callback: () => {
        void this.stripBoardBacklinks()
      }
    })
    ...
    this.addCommand({
      id: 'analyst-toolbox',
      name: 'Analyst toolbox on the selection',
      editorCallback: (editor) => {
        void openToolbox(this, editor)
      }
    })

    this.addCommand({
      id: 'analyse-email-headers',
      name: 'Analyse email headers',
      callback: () => {
        openHeaderAnalysis(this)
      }
    })
   ...
   */
  /**
   * Delete the `Project: [[Board]]` line from every task note that still has
   * one, in one pass, so turning the setting off does not mean waiting for each
   * note to be re-saved.
   *
   * Deliberately a TEXT edit, not a re-serialize: re-saving every note would
   * restamp 140 `updatedAt` values and destroy the Updated column's sort in one
   * batch. Only whole lines matching the generated shape are removed, and a
   * file that has none is left untouched, so the pass is idempotent and never
   * rewrites a note it did not change.
   */
  private async stripBoardBacklinks(): Promise<void> {
    const files = this.app.vault.getMarkdownFiles()
    const targets: TFile[] = []
    for (const file of files) {
      const cache = this.app.metadataCache.getFileCache(file)
      // Task notes only — the marker the hydrator keys on.
      if (!cache?.frontmatter || cache.frontmatter['pm-task'] !== true) continue
      const text = await this.app.vault.cachedRead(file)
      if (BOARD_BACKLINK_RE.test(text)) targets.push(file)
    }
    if (!targets.length) {
      this.showNotice('No task notes carry a board backlink line.')
      return
    }
    const ok = await confirmDialog(
      this.app,
      `Remove the "Project: [[…]]" line from ${targets.length} task note(s)?\n\n` +
        'Only that line is removed. Nothing else in the note changes, and no timestamps are restamped. ' +
        "The plugin never used the line — it finds a task's board from its folder.",
      'Remove them'
    )
    if (!ok) return
    let changed = 0
    for (const file of targets) {
      await this.app.vault.process(file, (text) => {
        const next = text.replace(BOARD_BACKLINK_RE, '').replace(/\n{3,}$/, '\n')
        if (next !== text) changed++
        return next
      })
    }
    this.showNotice(`Removed the board backlink from ${changed} note(s)`)
  }
